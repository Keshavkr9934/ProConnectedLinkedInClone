...
import { setIsTokenThere } from "@/config/redux/reducer/authreducer/index.js";
...
const dashboard = () => {
  ...
  useEffect(() => {
    if (authState.isTokenThere) {
      ...
      //   console.log("Fetching user data..." + authState.profileFeched);
    }
  }, [authState.isTokenThere]);
  const [postContent, setPostContent] = useState("");
  const [fileContent, setFileContent] = useState(null);
  // const [likes, setLikes] = useState(postState.likes);
  // console.log("Post State:", postState.message, postState.posts);

  const handlePostCreation = () => {
    if (postContent.length > 0 || fileContent) {
      dispatch(
        createPost({
          body: postContent,
          file: fileContent,
  ...
  useEffect(() => {
    if (postState.message) {
      // alert(postState.message);
      if (postState.message === "Post created successfully!") {
        setPostContent("");
        setFileContent(null);
      }
      dispatch(getAllPosts());
    }
  }, [postState.message, dispatch]);
  // console.log(postState.message);
              ...
              {postState.posts.map((post) => {
                return (
                  <div className={style.post} key={post._id}>
                    <div className={style.sigleCard}>
                      <img
                        src={`${BASE_URL}/${authState.user.userId.profilePicture}`}
                        alt="profilePIc"
                      />
                      <div className={style.sindlecard_profile}>
                    ...
                    <div className={style.icons}>
                      <div className={style.icon}>
                        <div className={style.icon_first}>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                          ...
                          <p className={style.likes}>{post.likes}</p>
                        </div>
                        <div className={style.icon_first}>
                          <svg
                            onClick={async () => {
                        ...
                        <div className={style.icon_first}>
                          <svg
                            onClick={() => {
                              ...
                              const urI = encodeURIComponent("apnaCollege");
            ...
            >
              ...
              >
                {postState.comments.comments.length === 0 && (
                  <h2 style={{ margin: "1rem" }}>No comments yet</h2>
                )}
                {/* {console.log("Comments:", postState.comments.comments.length)} */}
                {postState.comments.length !== 0 && (
                  <div className={style.postcomments}>
                    {/* {console.log(postState.comments.comments)} */}
                    {/* <p>{postState}</p> */}
                    {postState.comments.comments.map((postComment) => {
                      return (
                        <div
                          ...
                          key={postComment._id}
                        >
                          <div className={style.singleComment_profilePicture}>
                            <img
                              src={`${BASE_URL}/${postComment.userId.profilePicture}`}
                              alt="profilePic"
                            />
                            <div>
                              <p className={style.names}>{postComment.userId.name}</p>
                              <p className={style.usernames}>@{postComment.userId.username}</p>
                            </div>
                          </div>
                          <p className={style.commentBody}>{postComment.commentbody}</p>
                        </div>
                      );
                    })}
                  </div>
